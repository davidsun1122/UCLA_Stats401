...
iris_nb <- function(test, trainx, trainy){
  ...
  Species<-trainy
  train <- cbind(data.frame(trainx), Species)
  ...
  summary <- train %>% group_by(Species) %>%
    summarise(msl = mean(Sepal.Length), ssl = sd(Sepal.Length),
              ...
              mpl = mean(Petal.Length), spl = sd(Petal.Width),
  ...
  # # these will serve as the 'prior' probabilities of each class
   pr_set <- sum(train$Species=="setosa")/nrow(train)# proportion of class setosa in the training data
   pr_ver <- sum(train$Species=="versicolor")/nrow(train)
   pr_vir <- sum(train$Species=="virginica")/nrow(train)
  ...
  return(summary) # what the function outputs
}
...
s<-iris_nb(test_case_a, training_x, training_y)
