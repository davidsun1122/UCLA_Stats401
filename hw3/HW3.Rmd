...
The summary of the fitted GAM model 3 indicates that `horsepower` and `weight` variables have a significant relationship with `mpg`. `displacement` variable does not have a significant relationship with `mpg`, thus I will not include in the model 
...
#  plot(gam_fit4, se = TRUE)
  gam_fit4 <- gam(mpg ~ s(horsepower, 6) + s(weight, 2) +s(acceleration,2), data = Auto)
  summary(gam_fit4_a)
  plot(gam_fit4_a, se = TRUE)
```


Inspection of the plots, however, seems to show that perhaps the relationship between `acceleration` and `mpg` becomes linear if we have already included `weight` and `displacement` in the Tmodel. We can check this by fitting another gam where `acceleration` is added as a linear variable.


```{r, error = TRUE}
#  gam_fit5 <- gam(mpg ~ s(horsepower, 6) + s(weight, 2) + s(displacement, 4)+acceleration, data = Auto)
#  summary(gam_fit5)
  gam_fit5_a <- gam(mpg ~ s(horsepower, 6) + s(weight, 2) +acceleration, data = Auto)
  summary(gam_fit5_a)
  plot(gam_fit5, se = TRUE)
  anova(gam_fit5_a, gam_fit4)
```

From the above ANOVA table : `gam_fit5` uses a linear fit for acceleration and `gam_fit4` uses a spline fit for acceleration. The resulting p-value gives 0.1698. This is large enough for us to conclude that the relationship between acceleration and mpg is better modeled as a linear relationship.
