...
The summary of the fitted GAM model 3 indicates that `horsepower` and `weight` variables have a significant relationship with `mpg`.We can see `displacement` variable does not have a significant relationship with `mpg`, but we will keep it for now and compare at the end.  
...
```{r, error = TRUE}
  gam_fit5_a <- gam(mpg ~ s(horsepower, 6) + s(weight, 2) +s(displacement, 4)+acceleration, data = Auto)
  summary(gam_fit5_a)
...
The we compare the models of include or exclude `displacment` variable

```{r,error= TRUE}
  gam_fit5_b <- gam(mpg ~ s(horsepower, 6) + s(weight, 2) +acceleration, data = Auto) # gam_fit5_b model excludes `displacement`
  summary(gam_fit5_b)
  plot(gam_fit5_b, se = TRUE)
  anova(gam_fit5_b, gam_fit5_a)
```

From the above ANOVA result, we have very small p-value on gam_fit5_b, thus we will keep `displacement` variable in our model. 
