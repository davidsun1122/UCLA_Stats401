...
he summary of the fitted GAM model indicates that both variables have a significant relationship with `mpg`.
...
```{r, error = TRUE}
  gam_fit3 <- gam(mpg ~ s(horsepower, 6) + s(weight, 2) +s(displacement, 2), data = Auto)
  summary(gam_fit3)
  plot(gam_fit3, se = TRUE)
  
```

The summary of the fitted GAM model 3 indicates that `horsepower` and `displacement` variables have a significant relationship with `mpg`, but `weight` variable does not have a significant relationship with 'mpg'.
...
```{r, error = TRUE}
  gam_fit4 <- gam(mpg ~ s(horsepower, 6) + s(weight, 2) + s(displacement, 2)+s(acceleration,2), data = Auto)
  summary(gam_fit4)
  plot(gam_fit4, se = TRUE)
```

Inspection of the plots, however, seems to show that perhaps the relationship between `acceleration` and `mpg` becomes linear if we have already included `horsepower`,`weight` and `displacement` in the Tmodel. We can check this by fitting another gam where `weight` is added as a linear variable.


```{r, error = TRUE}
  gam_fit5 <- gam(mpg ~ s(horsepower, 6) + s(weight, 2) + s(displacement, 2)+acceleration, data = Auto)
  plot(gam_fit5, se = TRUE)
  anova(gam_fit5, gam_fit4)
```

From the above ANOVA table : `gam_fit5` uses a linear fit for acceleration and `gam_fit4` uses a spline fit for acceleration. The resulting p-value gives 0.2135. This is large enough for us to  conclude that the relationship between acceleration and mpg is better modeled as a linear relationship.
...
iris_nb <- function(test, trainx, trainy){
  ...
  summary <- train %>% group_by(Species) %>% 
