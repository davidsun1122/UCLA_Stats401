...
distances <- function(point, means,i){
 # point<- dat_df[1,1:2]
#  means<- centroids[,2:3]
  dist_to_centroid_A <- sqrt(((point-means[i,])[1])^2+((point-means[i,])[2])^2)
  #dist_to_centroid_B <- sqrt(((point-means[i,])[1])^2+((point-means[i,])[2])^2)
  #dist_to_centroid_C <- sqrt(((point-means[i,])[1])^2+((point-means[i,])[2])^2)
  #return(c(A = dist_to_centroid_A, B = dist_to_centroid_B, C = dist_to_centroid_C))
  return(dist_to_centroid_A)
}
