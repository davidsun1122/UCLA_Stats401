...
Weight Matrix 1: 12 parameters

bias values for hidden layer 1 : 3 parameter

weight matrix 2: 3 parameters

bias values for hidden layer 2 : 2 parameter
...
vectorize <- function(species){
  
  k<-rep (0,3)
  
  iris_new<-factor(iris$Species, levels = c(1,2,3), labels =c("setosa","versicolor","virginica"))
  
#  j<-as.integer(iris$Species[100])
  j<-as.integer(species)
  k[j] <- 1
...
W_2 <- matrix(1.nrow = input_layer_size, ncol = hidden_layer_size)
```
